...
mail-helper是一个基于imap/smtp协议，使用python3.x开发的邮件接收并解析、邮件发送工具，具体提供了哪些方法请移步imap_helper.py/smtp_helper.py，用户可在自己的项目中直接引入、开箱即用，或者结合flask等web框架轻松做成http接口供前端调用、把邮箱管理集成到自己的系统中，亦可通过注册中心（Eureka、Consul、Nacos等）的加持，做成微服务供其他系统调用。
...
# 设置为已读，返回布尔值
result = imap_helper.mark_seen_by_uids('inbox', '1564,1565')
print(result)

# 设置为未读，返回布尔值
result = imap_helper.mark_unseen_by_uids('inbox', '1564,1565')
print(result)
...
mail_subject='我是标题'
mail_content='我是内容'
result = smtp_helper.send_mail(receivers, mail_subject, mail_content)
print(result)
...
mail_subject='我是标题'
mail_content='我是内容'
result = smtp_helper.send_mail(receivers, mail_subject, mail_content, cc, bcc)
print(result)
